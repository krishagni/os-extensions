...
create_template() {
  ...
  echo "DATA_DIR=/opt/openspecimen/mysql-data" >> database.properties
  echo -e "\n#MySQL database backup directory." >> database.properties
  echo "BACKUP_DIR=/opt/openspecimen/mysql-database-backup" >> database.properties
}
...
check_db_props() {
  ...
  backup_dir=$(grep "^BACKUP_DIR=" $config_file | cut -d'=' -f2 | sed 's/ //g');
...
run_mysql() {
  ...
  docker run -d --name=$name \
	  ...
	  -v $backup_dir:/opt/openspecimen/nightly-db-backup \
